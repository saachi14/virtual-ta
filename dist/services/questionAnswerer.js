...
exports.TDSQuestionAnswerer = void 0;
...
class TDSQuestionAnswerer {
    constructor() {
        ...
        this.searchEngine = new embeddings_1.EmbeddingSearch();
        this.specificAnswers = {
            ...
            docker_podman: {
                ...
                answer: "While you can use Docker if you're familiar with it, we recommend using Podman for this course as it's what we'll be teaching and supporting. Docker is acceptable if you prefer it.",
                required_links: ["https://tds.s-anand.net/#/docker"]
            },
            exam_date: {
                keywords: ["tds", "sep", "2025", "exam", "end-term"],
                answer: "I don't know when the TDS Sep 2025 end-term exam is scheduled, as this information is not available yet. Please check the official announcements closer to the course start date.",
                required_links: []
            }
        };
    }
    ...
    matchSpecificQuestion(question) {
    ...
    async generateAnswerWithGPT(question, similarPosts) {
...
${context}`;
        try {
            ...
            return (similarPosts[0]?.post.cleaned_content.substring(0, 200) + "..." ||
    ...
    async answerQuestion(question) {
        ...
        if (specificMatch) {
            ...
            const links = specificMatch.required_links.map((url) => {
                ...
                const matchingPost = this.searchEngine["postsData"]?.find((post) => post.topic_url.includes(url) || url.includes(post.topic_url));
                return {
                    url,
        ...
        const similarPosts = await this.searchEngine.searchSimilarPosts(question, 10);
        const relevantPosts = similarPosts.filter((post) => post.similarity > 0.3);
        const postsToUse = relevantPosts.length > 0 ? relevantPosts : similarPosts.slice(0, 3);
...
exports.TDSQuestionAnswerer = TDSQuestionAnswerer;
//# sourceMappingURL=questionAnswerer.js.map